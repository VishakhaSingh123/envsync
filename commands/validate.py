...
def validate(ctx, env):
    ...
    for runtime, required in runtimes.items():
        ...
        if not installed:
            print(Fore.RED + f"  [X] {runtime:<12} required: {required:<12} installed: not found")
            all_pass = False
        elif version_matches(installed, required):
            print(Fore.GREEN + f"  [OK] {runtime:<12} required: {required:<12} installed: {installed}")
        else:
            print(Fore.RED + f"  [X] {runtime:<12} required: {required:<12} installed: {installed}")
            all_pass = False
    ...
    if all_pass:
        ...
        print(Fore.RED + "[X] Runtime version mismatch detected.")
...
def init_cmd(ctx):
    ...
    try:
        ...
        print(Fore.RED + f"[ERROR] Failed to scaffold config: {e}")
        sys.exit(1)
