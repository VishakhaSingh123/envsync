...
def audit(ctx, env, fail_on_missing, threshold):
    ...
    try:
        ...
        print(Fore.RED + f"[ERROR] Failed to load environment '{env}': {e}")
        sys.exit(1)

    try:
        truth = load_source_of_truth(config)
    except Exception as e:
        print(Fore.RED + f"[ERROR] Failed to load source of truth: {e}")
        sys.exit(1)
    ...
    if empty:
        print(Fore.YELLOW + "\n[EMPTY] EMPTY / PLACEHOLDER keys:")
        for k in empty:
            print(Fore.YELLOW + f"   ~ {k}")

    if extra:
        print(Fore.BLUE + "\n[EXTRA] EXTRA keys (not in .env.example):")
        for k in extra:
            print(Fore.BLUE + f"   + {k}")

    print()
    print(Fore.GREEN + f"[OK] OK: {ok} keys")
    print(Fore.RED + f"[X] Missing: {len(missing)} keys")
    ...
    if threshold > 0 and total_drift > threshold:
        print(Fore.RED + f"\n[FAIL] AUDIT FAILED: Drift count {total_drift} exceeds threshold {threshold}")
        sys.exit(1)

    if fail_on_missing and missing:
        print(Fore.RED + "\n[FAIL] AUDIT FAILED: Missing keys detected")
        sys.exit(1)

    if total_drift == 0:
        print(Fore.GREEN + f"\n[OK] Environment '{env}' is clean!")
    else:
        print(Fore.YELLOW + f"\n[WARN] Environment '{env}' has {total_drift} issues")
