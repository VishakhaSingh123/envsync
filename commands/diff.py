...
def diff(ctx, env1, env2, output):
    ...
    try:
        ...
        print(Fore.RED + f"[ERROR] Failed to load environment '{env1}': {e}")
        sys.exit(1)

    try:
        target = load_environment(config, env2)
    except Exception as e:
        print(Fore.RED + f"[ERROR] Failed to load environment '{env2}': {e}")
        sys.exit(1)
    ...
    if has_drift(entries):
        print(Fore.YELLOW + f"\n[WARN] Drift detected: {missing_count(entries)} missing, "
              f"{mismatch_count(entries)} mismatched, {extra_count(entries)} extra")
        sys.exit(2)
    else:
        print(Fore.GREEN + "\n[OK] Environments are in sync!")
